...
public:
    ...
    const Emitter* getEnvLights() const { return env; };
     ...
     */
    bool rayIntersect(const Ray3f &ray, Intersection &its,bool ignoreMedium=true) const {
        bool t= m_bvh->rayIntersect(ray, its, false,ignoreMedium);
        return t;
    }
     ...
     * \return \c true if an intersection was found
     */
    bool rayIntersect(const Ray3f &ray,bool ignoreMedium=true) const {
        Intersection its; /* Unused */
        return m_bvh->rayIntersect(ray, its, true,ignoreMedium);
    }
    ...
    Medium* lookMedium(std::string name) const
    {
        auto iter = medium_map.find(name);
        if (iter != medium_map.end())
            return iter->second;
        else
            return nullptr;
    }
