...
public:
    ...
    virtual Color3f eval(const BSDFQueryRecord &bRec) const override {
        ...
        float beck = evalBeckmann(wh);
        float fres = fresnel(wh.dot(bRec.wi), m_extIOR, m_intIOR);
        float G = smithBeckmannG1(bRec.wi, wh) * smithBeckmannG1(bRec.wo, wh);
        float dom = 4* Frame::cosTheta(bRec.wo)*Frame::cosTheta(bRec.wi);
        
        return m_kd * INV_PI + m_ks * beck * fres * G / dom;
    }
    ...
    virtual float pdf(const BSDFQueryRecord &bRec) const override {
        ...
        return m_ks * D * Frame::cosTheta(wh) * Jh + (1 - m_ks) * cos * INV_PI;
    }
